...
def formatar_poly1d(p):
    """Transforma o objeto poly1d do numpy em uma string matemática legível."""
    termos = []
    coeficientes = p.coeffs
...
def interpolacao_lagrange(pontos, arquivo_saida):
    """Calcula o polinômio interpolador usando o Método de Lagrange."""
    n = len(pontos)
    ...
    for i in range(n):
        ...
        # O laço 'j' constrói os multiplicadores (x - x_j) / (x_i - x_j)
        for j in range(n):
            if i != j:
...
def interpolacao_newton(pontos, arquivo_saida):
    """Calcula o polinômio interpolador usando o Método de Newton (Diferenças Divididas)."""
    n = len(pontos)
    x = [p[0] for p in pontos]
    y = [p[1] for p in pontos]
    ...
    # Criação de uma matriz vazia para a tabela de diferenças
    dd = [[0.0] * n for _ in range(n)]
    
    # A primeira coluna é formada pelas imagens f(x)
    for i in range(n):
        dd[i][0] = y[i]
        
    # Preenchendo as ordens seguintes da tabela
    for j in range(1, n):
        for i in range(n - j):
            dd[i][j] = (dd[i+1][j-1] - dd[i][j-1]) / (x[i+j] - x[i])
            
    # Gravando a tabela de diferenças divididas no relatório
    ...
    # A primeira linha da tabela (dd[0]) contém os coeficientes da equação de Newton
    coeficientes = dd[0] 
    ...
    polinomio_final = np.poly1d(0.0)
    termo = np.poly1d(1.0) # Começa multiplicando por 1
    ...
    for i in range(n):
        ...
        # Prepara o multiplicador para a próxima iteração do laço
        if i < n - 1:
...
def ler_arquivo(caminho):
    """Lê os pontos ignorando linhas em branco."""
    pontos = []
