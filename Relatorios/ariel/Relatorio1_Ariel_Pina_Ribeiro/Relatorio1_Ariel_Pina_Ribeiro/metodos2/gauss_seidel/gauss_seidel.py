...
def gauss_seidel(A: np.ndarray, b: np.ndarray, tol: float = 1e-6, max_iter: int = 1000) -> Tuple[Optional[np.ndarray], int]:
    n = b.size
    if A.shape != (n, n):
        raise ValueError("Dimensões de A e b incompatíveis.")

    if np.any(np.isclose(np.diag(A), 0.0)):
        raise ZeroDivisionError("Zero (ou próximo de zero) detectado na diagonal de A; Gauss–Seidel inválido.")

    x = np.zeros(n, dtype=float)

    for k in range(1, max_iter + 1):
        for i in range(n):
            s1 = float(np.dot(A[i, :i], x[:i])) if i > 0 else 0.0
            s2 = float(np.dot(A[i, i+1:], x[i+1:])) if i + 1 < n else 0.0
...
def main(caminho_entrada: str = "../entrada.txt"):
    ...
    t0 = time.time()
    try:
        x, iteracoes = gauss_seidel(A, b, tol=tolerancia, max_iter=max_iter)
    except Exception as exc:
        print(f"Erro durante execução do método de Gauss–Seidel: {exc}")
        raise
    tempo = time.time() - t0
